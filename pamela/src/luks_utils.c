...
int luks_create(const char *id, const char *container_file, const char *key)
{
  ...
  err_if(shell_run("echo %s | cryptsetup luksFormat %s", key, container_file));
  err_if(shell_run("echo %s | cryptsetup luksOpen %s container_%s", key,
		   container_file, id));
  err_if(shell_run("mkfs.ext4 /dev/mapper/container_%s", id));
  err_if(shell_run("cryptsetup luksClose container_%s", id));
  return 0;
}
...
{
  err_if(shell_run("echo %s | cryptsetup luksOpen %s container_%s",
		   key, container_file, id));
  err_if(shell_run("mkdir -p %s", mountpoint));
  err_if(shell_run("mount /dev/mapper/container_%s %s", id, mountpoint));
...
{
  err_if(shell_run("umount %s", mountpoint));
